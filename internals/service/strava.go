...
func (s *StravaService) Auth(input *models.AuthHandler) (int, *models.StravaUser, error) {
	...
	if err != nil {
		s.logger.Errorf("error validate auth model: %v", err)
		return code, nil, err
	}
	...
	if err != nil {
		s.logger.Errorf("error generate json to make strava auth: %v", err)
		return http.StatusInternalServerError, nil, err
	}

	req, err := http.NewRequest("POST", strava_auth_url, bytes.NewBuffer(json_request))
	if err != nil {
		s.logger.Errorf("error POST request strava auth: %v", err)
		return http.StatusInternalServerError, nil, err
	}
	...
	if err != nil {
		s.logger.Errorf("error make request strava auth: %v", err)
		return http.StatusInternalServerError, nil, err
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		s.logger.Errorf("autorization error: %v", err)
	...
	if err != nil && err != io.EOF {
		s.logger.Errorf("error unmarshal athlete model: %v", err)
		return http.StatusInternalServerError, nil, err
	}
...
func (s *StravaService) UploadActivity(file string, id int64) error {
	...
	if err != nil {
		s.logger.Errorf("error opening file %s: %v", file, err)
		return err
	}
	defer data.Close()
	...
	if err != nil {
		s.logger.Errorf("error create part: %v", err)
		return err
	}
	_, err = io.Copy(part, data)
	if err != nil {
		s.logger.Errorf("error copy file to part: %v", err)
		return err
	}
	...
	if err != nil {
		s.logger.Errorf("error add 'data_type' field: %v", err)
		return err
	}
	err = writer.Close()
	if err != nil {
		s.logger.Errorf("error close writer: %v", err)
		return err
	}

	req, err := http.NewRequest("POST", strava_api_url+"uploads", body)
	if err != nil {
		s.logger.Errorf("error create upload request: %v", err)
		return err
	}
	...
	if err != nil {
		s.logger.Errorf("error refresh token: %v", err)
		return err
	}

	token, err := s.rep.GetAccesToken(id)
	if err != nil {
		s.logger.Errorf("error get acces token from DB: %v", err)
		return err
	}
	bearer := "Bearer " + token
	...
	resp, err := client.Do(req)
	if err != nil {
		s.logger.Errorf("error do request (upload file to strava): %v", err)
		return err
	} else {
		...
		err = json.Unmarshal(body, &res)
		if err != nil && err != io.EOF {
			s.logger.Errorf("error response processing aftet upload file to strava: %v", err)
			return err
		}
		resp.Body.Close()
...
func (s *StravaService) getNewToken(id int64) error {
	...
	if err != nil {
		s.logger.Errorf("error get refresh token from DB: %v", err)
		return err
	}
	...
	req, err := http.NewRequest("POST", strava_auth_url, bytes.NewBuffer(json_request))
	if err != nil {
		s.logger.Errorf("error creating request to strava (refresh token): %v", err)
		return err
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := client.Do(req)
	if err != nil {
		s.logger.Errorf("error do request to strava (refresh token): %v", err)
		return err
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		s.logger.Errorf("error get new refresh token, get http.status %d", resp.StatusCode)
	...
	err = json.Unmarshal(body, &res)
	if err != nil && err != io.EOF {
		s.logger.Errorf("error parse new refresh token: %v", err)
		return err
	}

	err = s.rep.RefreshToken(id, res)
	if err != nil {
		s.logger.Errorf("error write new refresh token to DB: %v", err)
		return err
	} else {
